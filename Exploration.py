...
class exploration_r:
    def __init__(self,start,goal,step_size,theta_s,theta_g,clearance,radius):
        ...
        self.image_p=np.zeros([round(300/self.threshold),round(400/self.threshold),round(360/self.theta_diff)])
    ...
    def action_space(self,image,pos_0,pos_1,theta_d):
        ...
        for theta in range(-60,60+self.theta_diff,self.theta_diff):
            ...
            if (pos_0 >=0+self.padding and pos_1>=0+self.padding) and (pos_0 <=299+self.padding and pos_1<=399-+self.padding):
                ...
                if (x >=0+self.padding and y>=0+self.padding) and (x <=299-self.padding and y<=399-self.padding):
                    
                    score=self.string(x,y,n)
                    
                    if np.array_equiv(image[299-x,y,:],np.array([00,00,0])) or self.image_p[round(x/self.threshold),round(y/self.threshold),int(n)]==2:
                        pass
                    elif self.image_p[round(x/self.threshold),round(y/self.threshold),int(n)]==1:
                        
                        if self.cost[score,round(theta_net/self.theta_diff)]>self.cost_togo[parent,round(theta_d/self.theta_diff)] +1+self.h(x,y):
                             
                             self.cost_togo[score,round(theta_net/self.theta_diff)]=self.cost_togo[parent,round(theta_d/self.theta_diff)]+1
                             self.cost[score,round(theta_net/self.theta_diff)]=self.cost_togo[score,round(theta_net/self.theta_diff)]+self.h(x,y)
                             self.frontier[x,y,round(theta_net/self.theta_diff)]=self.cost[score,round(theta_net/self.theta_diff)]
    ...
    def frontier_list(self,image):
        ...
        if self.image_p[round(pos_0/self.threshold),round(pos_1/self.threshold),n]==1 or np.array_equiv(image[299-pos_0,pos_1,:],np.array([00,00,0])) or self.image_p[round(pos_0/self.threshold),round(pos_1/self.threshold),n]==2  :
            ...
            if self.step_size==1:
                ...
                self.image_p[round(pos_0/self.threshold),round(pos_1/self.threshold),round(theta/self.theta_diff)]=1
